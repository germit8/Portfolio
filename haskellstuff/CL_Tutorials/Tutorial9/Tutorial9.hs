...
isDFA (FSM qs as ts ss fs) = (size ss == 1)
  && and [ length[ q' | q' <- toList qs, (q, a, q')`member`ts ] == 1 | q <- toList qs, a <- toList as ]
...
ddelta (FSM  qs as ts ss fs ) source sym = fromList [ t | it@(q, sym', t) <- toList ts, s <- toList source, q == s && sym == sym']

-- Q 11.

next :: (Ord q) => FSM q -> Set(Set(q)) -> Set(Set(q))
next fsm@(FSM qs as ts ss fs) supers = fromList [ ddelta fsm super sym | super <- toList supers, sym <- toList as ]

-- Q 12.

reachable :: (Ord q) => FSM q ->  Set(Set(q)) -> Set(Set(q))
reachable fsm@(FSM qs as ts ss fs) supers =
  let new = next fsm (fromList [transition ts super a | super <- toList supers, a <- toList as])
