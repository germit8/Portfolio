...
rowsNoRepetition = And [ Or [ N (i, j, n), N (i, j', n) ] | i <- [1..9], n <- [1..9], j <- [1..9], j' <- [1..(j-1)] ]
...
-- 2. sum ds ==  m.
--scores :: Int -> Int -> [[Int]]
--scores len sm = [ x | x <- subsequences [1..9], length x == len && sum x == sm]
...
Function from FP Tut6 for checking how many ways can we split the arrows
We get the base number without considering the unused literals, but then
we can just multiply the base number by 2^n where n is num of unused literals

numOfSplits :: Eq a => Wff a -> Int
numOfSplits p = length [ e | e <- envs (atoms p),  eval e p ]
...
mustSumTo k sh = And [ deny ( sh >>*< ss ) | k' <- [n..9*n], k' /= k, ss <- scores n k' ] 
 where n = length sh
{-
mustSumTo :: Int -> Int -> Shape -> Form (Int,Int,Int)
mustSumTo ind k shp | ind < length ss = And [Or [P (i, j, o)] | i <- [1..fst (head shp)], j <- [1..snd (head shp)], o <- ss !! ind] <&&> mustSumTo (ind+1) k shp
 where ss = scores (uncurry (*) (head shp)) k
 -}
