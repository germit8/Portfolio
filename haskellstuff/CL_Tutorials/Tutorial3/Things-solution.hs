...
(||=) :: [Predicate Thing] -> Predicate Thing -> Bool
as ||= b = and [ b x | x <- things, and [a x | a <- as] ]
...
-- > isBig &:& isAmber |= isDisc
-- False
-- > isBig &:& isDisc |= isAmber
-- True
-- > isSmall &:& neg isBlue |= neg isDisc
-- True
-- > isBig |:| isAmber |= neg isSquare
-- False
-- > neg (isSquare |:| isBlue) |= hasThickContour
-- True
-- > neg isSquare &:& neg isAmber |= isDisc
-- True
