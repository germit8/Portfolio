...
score :: Char -> Int
score x = delta (isAlpha x) + delta (isUpper x) + delta (toLower x `elem` "aeiou")
  where
...
contains str substr = or [ substr `isPrefixOf` s | s <- suffixes str ]
...
prop_contains str1 str2 | length str1 == length str2 = not (contains str1 (str2 ++ "abba"))
