...
exports.apiSvatky = function(req, res) {
    ...
    let obj = {}
    if (q.query["month"] && q.query["day"]) {
        let day = q.query["day"]
        let month = q.query["month"]

        obj.datumCesky = day + "." + month + ".";
        obj.dnesniSvatek = SVATKY[month][day]
    } else {
        let d = new Date();
        obj.systDatum = d;

        obj.datumCesky = date(d, "dd.mm.yyyy");
        obj.dnesniSvatek = SVATKY[parseInt(obj.datumCesky[3] + obj.datumCesky[4], 10)][parseInt(obj.datumCesky[0] + obj.datumCesky[1], 10)]
        obj.zitrejsiSvatek = SVATKY[parseInt(obj.datumCesky[3] + obj.datumCesky[4], 10)][parseInt(obj.datumCesky[0] + obj.datumCesky[1], 10) + 1]
    }
